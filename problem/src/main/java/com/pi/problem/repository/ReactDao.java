...
public interface ReactDao extends JpaRepository<React,Integer> {
    ...
    React findReactByUserAndComment(int id_user, int id_comment);
    ...
    boolean FindReactOfUser(int id_user, int id_comment);
    @Query("DELETE FROM React r WHERE r.id_user = :idUser AND r.comment.id_comment = :idComment")
    void deleteReactByUserAndComment(int idUser, int idComment);
}
